...
@Component({
  selector: 'app-first-year-student',
  templateUrl: './first-year-student.component.html',
  styleUrls: ['./first-year-student.component.css']
})
export class FirstYearStudentComponent implements OnInit {
  ...
  constructor(
    ...
    this.addEtudiantForm = this.fb.group({
      ...
      niveau: ['1', Validators.required],  // Set default value to '1'
    });
    ...
    this.updateEtudiantForm = this.fb.group({
      ...
      cne: ['', Validators.required],
      cin: ['', Validators.required],
      Niveau: ['1'] // Set the Niveau for first-year students
    });
  }
  ...
  fetchFirstYearEtudiants() {
    this.service.getEtudiantsByFiliereId(this.filiereId!).subscribe((etudiants) => {
      ...
      this.firstYearEtudiants = etudiants.filter((etudiant) => etudiant.niveau === '1');
    });
  }
