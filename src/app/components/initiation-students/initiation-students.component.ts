...
@Component({
  selector: 'app-initiation-students',
  templateUrl: './initiation-students.component.html',
  styleUrls: ['./initiation-students.component.css']
})
export class InitiationStudentsComponent {
    ...
    if (this.filiereId) {
      this.service.getStagesByFiliereId(this.filiereId).subscribe(data => {
        ...
        this.stages = data.reverse().filter(stage => {
          ...
          const isPFE = typeof stage.type === 'string' && stage.type.toLowerCase() === 'initiation';
